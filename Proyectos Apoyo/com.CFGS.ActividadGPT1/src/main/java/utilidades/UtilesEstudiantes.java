...
public class UtilesEstudiantes {
	...
	public void cargarDatosCSV(String nombreArchivoCSV) {
		try (BufferedReader br = new BufferedReader(new FileReader(rutaResources + nombreArchivoCSV))) {
			String line;
			br.readLine(); // Saltar la cabecera
			// Bucle para leer cada línea del archivo CSV.
			while ((line = br.readLine()) != null) {
				// Divide la línea en partes usando la coma como delimitador.
				// Cada parte representa un dato que será asignado a un atribut
				String[] valores = line.split(",");

				// Extrae String nombre y limpia los espacios extras
				String nombre = valores[0].trim();
				Integer edad = Integer.parseInt(valores[1].trim());
				String idEstudiante = valores[2].trim();
				String curso = valores[3].trim();

				// List<Integer> listaNumeros =
				// Arrays.asList(Integer.parseInt(valores[1].trim()),Integer.parseInt(valores[2].trim()));

				// Campo caso ENUM
				// Curso curso = Curso.valueOf(valvaloresues[6].trim().toUpperCase());

				estudiantes.add(new Estudiante(nombre, edad, idEstudiante, curso));
			}
